...
public class VodPlayerActivity extends Activity implements OnKeyListener {
	...
	private boolean canRestartstream = true;
	private String mAdUrl;
	/*
	 * �Ƿ����ڲ�����һ�� ������һ�� ��ǵ�һ�ν��벥����ʷ��¼
	 */
	private boolean isPlayDirect = false;
	public static final String sdkPath = Environment
			.getExternalStorageDirectory().toString() + "/" + mVodCacheName;
	...
	private TimerTask mTimerTaskCache = new TimerTask() {
		@Override
		public void run() {
			mForcePlayInfo = RequestUtil.getInstance().request(
					GET_FORCE_PLAY_INFO + mCurrentVideo.getChannelId());
			// logger.e("ChannelId="+mCurrentVideo.getChannelId());
			// logger.e(mForcePlayInfo);
			// Log.d("isplay",mVideoView.getBufferPercentage()+"");
	...
	private OnClickListener mDialogClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (v == mBtnSubmit) {
				Toast.makeText(VodPlayerActivity.this, R.string.start_download,
	...
	private Handler mUpdateHandler = new Handler() {
		public void handleMessage(Message msg) {
			File file = new File(upgradepath + "/files/", Configs.APK_NAME);
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			mOSDControl.setSecondProgress(currentPos
					+ (int) (playInfo[0] * 1000));
		}
	};
	private Timer mForceTimer = new Timer();
	private TimerTask mForceTimerTask = new TimerTask() {
	...
	private void RestartForce() {
		if (canRestartstream) {
			FinalHttp finalHttp = new FinalHttp();
			finalHttp.addHeader("User-Agent", "cwhttp/v1.0");
			Log.d("getStartUrl()", getStartUrl());
			finalHttp.get(getStartUrl(), null);
		}
	}

	private Long[] parseForcePlayInfo(String forcePlayInfo) {
		Document doc = null;
		// Log.d("xml",forcePlayInfo);
		try {
			String parseXml = mForcePlayInfo.replace(
					"<?xml version=\"1.0\" encoding=\"utf-8\"?>", "").replace(
					"ver=1.0", "");
			doc = DocumentHelper.parseText(parseXml);
			Node root = doc.getRootElement();
			Element element = (Element) root
					.selectSingleNode("/forcetv/channel");
			long cacheTime = Integer.valueOf(element.attribute("cache_time")
					.getValue());
			long traffic = Integer.valueOf(element.attribute(
					"download_flowkbps").getValue());
	...
	private void startVod() {
		if (mCurrentVideo.getUrl().indexOf("http") == 0) {
			...
			Log.d("httpStart", mCurrentVideo.getUrl());
			// setVideoScale(mode);
			getScreenSize();
			...
			setTitle();
			try {
				mTimerCache.schedule(mTimerTaskCache, START_CACHE_TIME * 1000,
						GET_CACHE_PEROID * 1000);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	private String getStartUrl() {
		...
		StringBuilder sb = new StringBuilder(
				"http://127.0.0.1:9906/cmd.xml?cmd=");
		sb.append(FORCE_CMD).append("&id=").append(chnnelId).append("&server=")
				.append(streamIp);
		...
		Log.d("forceOpenUrl", sb.toString());
	...
	private void getIntentData() {
		...
		if (mPlayVodPos < mListVideos.size()) {
			...
			logger.i("Player : Streamip = " + vodVideo.getStreamip()
					+ " ChannelId = " + vodVideo.getChannelId() + " Type = "
					+ vodVideo.getType() + " Link = " + vodVideo.getLink()
	...
	private void initWidget() {
		...
		mOSDControl = new OSDControlBar(VodPlayerActivity.this, mContainer,
				mVideoView);
	...
	private OnTouchListener mVideoViewTouchListener = new OnTouchListener() {
		...
		public boolean onTouch(View v, MotionEvent event) {
			if (isLoadingVod)
				return false;
			int visibility = View.VISIBLE == mOSDControl.getVisibility() ? View.GONE
					: View.VISIBLE;
		...
		public void handleMessage(android.os.Message msg) {
			...
			if (Configs.SUCCESS == msgWhat) {
				mVideoPath = new StringBuilder(PLAY_SERVER)
						.append(mCurrentVideo.getChannelId()).append(PERIOD)
						.append(mCurrentVideo.getType()).toString();
				logger.i(mVideoPath);
				getScreenSize();
				Log.d("playurl", mVideoPath);
				int mode = DisplayMode.getMode(VodPlayerActivity.this);
				setVideoScale(mode);
				...
				setTitle();
				try {
					mTimerCache.schedule(mTimerTaskCache,
	...
	public boolean sendHttpRequest(String channelId, String streamIp,
			String cmd, String link) {
			...
			case 258:
				...
				if (null == ct) {
					ct = new CustomToast(getApplicationContext(),
							getString(R.string.video_pro)
									+ mOSDControl.getDisplayMode(), 24);
					ct.show();
				} else {
					ct.cancel();
					ct = new CustomToast(getApplicationContext(),
	...
	private void closeForce() {
		new FinalHttp().get(CLOSE_FORCE, new AjaxCallBack<Object>() {
			...
			public void onSuccess(Object t) {
				...
				// Toast.makeText(getApplicationContext(), "ԭ���رճɹ�",
				// Toast.LENGTH_LONG).show();
			}
			...
			public void onFailure(Throwable t, int errorNo, String strMsg) {
				super.onFailure(t, errorNo, strMsg);
				logger.i("ԭ��Դ�ر�����ɹ� = " + t.toString());
				// Toast.makeText(getApplicationContext(), "ԭ���ر�ʧ��",
				// Toast.LENGTH_LONG).show();
			}
		});
	}
